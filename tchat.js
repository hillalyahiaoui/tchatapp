...
(function setupEmojiPicker() {
 ...
 document.getElementById('testCam').addEventListener('click', async () => {
  try {
    ...
    console.log("✅ Flux caméra et micro OK", stream);
  } catch (err) {
    console.error("❌ Erreur flux caméra/micro:", err);
...
// ======================== tchat.js (version finale - debug alerts) ========================
// Inclure dans le HTML : <script type="module" src="./tchat.js"></script>
...
var containeraudio=document.querySelector('.containeraudio');
var areacoeur=document.querySelector('.areacoeur');
var testCamBtn = document.getElementById('testCam');
...
// ----------------- UPLOAD & SEND FILE TO STORAGE (avec alertes debug) -----------------
async function uploadAndSendFile(file){
  if(!file){
  ...
  // sécuriser le nom de fichier (enlever caractères spéciaux)
  const safeName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, "_");
  const path = `${type}s/${Date.now()}_${safeName}`;
  const sref = storageRef(storage, path);

  try {
    alert("📤 Upload en cours : " + file.name);
    console.log("Upload start:", path, file);

    // upload
    const snapshot = await uploadBytes(sref, file);
    alert("✅ Upload terminé sur Storage");
    console.log("Upload snapshot:", snapshot);

    // récupération URL
    const url = await getDownloadURL(snapshot.ref);
    alert("🔗 URL récupérée : " + url);
    console.log("DownloadURL:", url);

    // push message vers Realtime DB
    await push(messagesRef, {
      user: currentUserName,
      type: type,
      url: url,
    ...
    alert("📨 Message envoyé dans la base avec succès !");
    console.log("File message pushed for", currentUserName, url);
  } catch (err) {
...
onChildAdded(messagesRef, (snap) => {
  ...
  try{
    ...
    // IMAGE
      ...
      const imgHtml = `<img class="phshared" src="${escapeHtml(msg.url)}" loading="lazy" />`;
      if(msg.user === currentUserName){
        element.innerHTML=`
          <div class="bloc1">
            <div class="date">${escapeHtml(dateText)}</div>
            <div class="hillal">
              <div class="droite">${imgHtml}</div>
              <img class="ph" src="${escapeHtml(avatar[msg.user]||picture[0])}">
            </div>
          </div>
        `;
...
if(filInput){
  filInput.addEventListener("change", (e)=>{
    ...
    if(file){
      ...
      if(lastPreviewElement){ lastPreviewElement.remove(); lastPreviewElement = null; }
      ...
      if(file.type.startsWith("image/")){
        ...
        reader.onload = function(ev){
          ...
          content.scrollTop = content.scrollHeight;
          lastPreviewElement = previewWrap;
        };
        reader.readAsDataURL(file);
      } else {
        // preview pour autres types (audio/video) si nécessaire
        if(file.type.startsWith("audio/")){
          const preview = document.createElement('div');
          preview.textContent = "✅ Audio prêt à envoyer: " + file.name;
          content.appendChild(preview);
          lastPreviewElement = preview;
        } else if(file.type.startsWith("video/")){
          const preview = document.createElement('div');
          preview.textContent = "✅ Vidéo prêt à envoyer: " + file.name;
          content.appendChild(preview);
          lastPreviewElement = preview;
        }
      }
...
if(envoyer){
  envoyer.addEventListener("click", async (e)=>{
    ...
    // Si un fichier est en attente, on l'envoie (image/audio/video)
    if(pendingFile){
      await uploadAndSendFile(pendingFile);
      pendingFile = null;
      if(lastPreviewElement){ lastPreviewElement.remove(); lastPreviewElement = null; }
    ...
    if(messageText !== ""){
      try{
        ...
        console.error("Error push text:", e);
        alert("Erreur envoi message texte : " + (e.message||e));
      }
    }
